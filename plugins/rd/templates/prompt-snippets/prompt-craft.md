...
> 本文件说明 `docs/prompt/` 下各 Prompt 文件的用途与统一结构，供团队维护参考。由 `/rd:init` 生成。
...
| 文件 | 内容 | 主要消费命令 |
|------|------|-------------|
| `architecture.md` | 项目架构（分层、技术栈、目录、命名）| `/rd:dev`、`/rd:test`（自动生成）|
| `code-generation.md` | 代码生成规范 | `/rd:dev` |
| `refactoring.md` | 重构规范（不改变行为）| `/rd:do` |
...
> 本规范由 `/rd:init` 自动生成。新增自定义 Prompt 文件时沿用同一 5 节结构。
