---
name: specs
description: 规范文档管理 - 项目级公共知识层，集中沉淀散落在代码中的枚举、规则、契约
---

# 规范文档管理

执行模型：按[命令模型路由](../../shared/_command-models.md)中 `rd:specs` 的档位执行；已作为执行子代理时不再次路由。
...
| 子命令 | 说明 | 权限 | 示例 |
|--------|------|------|------|
| (空) | 列出所有规范文档 | 所有角色 | `/rd:specs` |
| `new` | 创建规范文档 | 仅 primary | `/rd:specs new 订单数据类型` |
| `show` | 查看规范文档 | 所有角色 | `/rd:specs show order-types` |
| `edit` | 编辑规范文档 | 仅 primary | `/rd:specs edit order-types` |
...
- **写入位置**：primary 直接写需求根目录的 `specs/`；readonly 只能读取绑定主仓的现有规范。
...
/rd:specs show <名称> · /rd:specs new <名称> · /rd:specs edit <名称>
```

无文档时提示：`/rd:specs new <名称>`
