...
执行模型：按[命令模型路由](../../shared/_command-models.md)中 `rd:help` 的档位执行；已作为执行子代理时不再次路由。
...
```
/rd:help [章节名或编号] [--lang=zh|en|ko]
```

示例：
- `/rd:help` → 中文章节索引（或 settings.local.json 中设定的语言）
- `/rd:help --lang=en` → English section index
- `/rd:help --lang=ko` → 한국어 섹션 인덱스
- `/rd:help 4` → 第四章「开发阶段」
- `/rd:help 4 --lang=en` → Section 4 "Development"
- `/rd:help branch --lang=en` → English match on "branch management"
...
后续所有 `/rd:help`、`/api:help`、`/pm:help` 都会默认用该语言。
