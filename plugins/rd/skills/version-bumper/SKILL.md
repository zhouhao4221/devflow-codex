---
name: version-bumper
description: 版本号管理助手。仅在执行 /rd:release 命令时触发。 自动检测各插件变更范围，按 semver 规则推导并更新插件版本号和整体版本号。
---

# 版本号管理助手

仅在 `/rd:release` 命令执行时触发，负责在发版前更新 devflow 各插件版本号。

## 触发条件

仅在 `/rd:release` 执行时触发，在 release 流程的步骤 2（确定版本号）之后、步骤 9（生成 changelog）之前执行。
...
| `plugins/rd/.codex-plugin/plugin.json` | `version` | req 插件独立版本 |
...
```bash
git add .agents/plugins/marketplace.json plugins/rd/.codex-plugin/plugin.json \
