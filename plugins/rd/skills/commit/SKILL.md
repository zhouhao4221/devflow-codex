...
执行模型：按[命令模型路由](../../shared/_command-models.md)中 `rd:commit` 的档位执行；已作为执行子代理时不再次路由。

生成符合 Conventional Commits 规范的 Git 提交，自动关联当前需求编号，便于后续 `/rd:changelog` 生成版本说明。
...
**示例：**
- `/rd:commit` — 根据实际变更生成提交信息并提交
- `/rd:commit 实现部门渠道关联` — 自动分析变更并生成提交
...
根据实际暂存 diff、用户消息和调用上下文选择类型（如上游为 `/rd:fix` 且实际为修复，则使用「修复」）。不要求提供提交消息或 `--auto` 才能自动选择。
...
本命令生成的提交消息使用中文前缀，`/rd:changelog` 可直接解析：
