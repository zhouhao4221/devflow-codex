...
执行模型：按[命令模型路由](../../shared/_command-models.md)中 `rd:update` 的档位执行；已作为执行子代理时不再次路由。
...
SOURCE_PATH="${CODEX_PLUGIN_ROOT:-}"

# 1b. 查找 Codex marketplace 缓存目录
...
```
❌ 无法定位插件目录。
   请检查 Codex marketplace 安装目录，或手动指定插件路径。
```
...
CURRENT_VER=$(jq -r '.version // "unknown"' "$SOURCE_PATH/plugins/rd/.codex-plugin/plugin.json" 2>/dev/null)
```
...
NEW_VER=$(jq -r '.version // "unknown"' "$SOURCE_PATH/plugins/rd/.codex-plugin/plugin.json" 2>/dev/null)
```
...
读取 `$SOURCE_PATH/plugins/rd/schemas/prompt-schema.md`，对 schema 中每个文件逐项检查：
...
Prompt 文件检查（schema v<VERSION>）：
  ...
  docs/prompt/architecture.md
    ✅ 技术栈      ✅ 分层      ✅ 目录      ❌ 命名规范（必需，/rd:dev 依赖）
    ⚠️  错误处理（推荐，/rd:dev 可能生成不一致的错误返回）

  docs/prompt/testing.md
    ⚠️  文件不存在（可选，/rd:test 将使用内置默认值）

建议：补充 1 个必需章节，详见 plugins/rd/schemas/prompt-schema.md
```
