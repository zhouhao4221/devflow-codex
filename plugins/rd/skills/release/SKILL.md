...
执行模型：按[命令模型路由](../../shared/_command-models.md)中 `rd:release` 的档位执行；已作为执行子代理时不再次路由。
...
> CLI 优先：GitHub → `gh`；Gitea → 检测 `tea`，不支持的接口回退 curl。详见 `_gitea_cli.md`（按需读取 [_gitea_cli.md](../../shared/_gitea_cli.md)）。
> 设计原理和边界情况详见 `release-rationale.md`（按需读取 [release-rationale.md](../../shared/release-rationale.md)）。
> **发布前置**：优先执行目标项目的发布规则。发布 DevFlow Codex 仓库自身时，运行 `python3 scripts/generate-codex-marketplace.py` 和 `./scripts/validate-skills.sh --ci`；涉及共享资源或适配器时再运行 `python3 scripts/test-export-skills.py`。`SKILL.md` 是作者源，command 包装由生成器派生。
...
示例：
- `/rd:release`（准备产物 + PR + draft Release，不打 tag）
- `/rd:release --tag`（同上 + annotated tag）
- `/rd:release --no-draft`（正式 Release，无 tag）
- `/rd:release --tag --no-draft`（正式 Release + tag）
- `/rd:release v1.2.0`（显式版本 + draft Release）
...
`PRE_RELEASE_CHECKS` 非空时逐条执行项目检查，打印每条结果（✅ 通过 / ❌ 失败）。**任意一条失败则硬停止**，不进入后续步骤。

另外，需求根目录可读时，先从 [_storage.md](../../shared/_storage.md) 解析唯一 primary 仓库，并运行[需求目录守卫](../../scripts/check-requirements.py)的 `--check --root <primary 仓库根>`。状态、生命周期或归档目录不一致时硬停止；需求目录确实不存在时报告检查不适用，不能宣称通过。
...
执行 `/rd:changelog` 核心逻辑，写入 `docs/changelogs/<version>.md`（已存在则覆盖）。
...
完整速查见 rationale §12（按需读取 [release-rationale.md](../../shared/release-rationale.md)）：
