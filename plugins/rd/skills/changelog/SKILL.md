...
执行模型：按[命令模型路由](../../shared/_command-models.md)中 `rd:changelog` 的档位执行；已作为执行子代理时不再次路由。
...
```
/rd:changelog <version> [--from=<tag|commit>] [--to=<tag|commit>]
```
...
**示例：**
- `/rd:changelog v1.2.0` — 从上一个 tag 到 HEAD
- `/rd:changelog v1.2.0 --from=v1.1.0` — 从 v1.1.0 到 HEAD
- `/rd:changelog v1.2.0 --from=v1.1.0 --to=v1.2.0` — 指定完整范围
- `/rd:changelog v1.2.0 --from=abc1234` — 从某个 commit 开始
...
> **被 `/rd:release` 调用时**：`/rd:release` 会显式传入 `--from`，跳过下面的自动检测链，draft 模式对本步无影响。
...
**为什么要 draft 感知**：v3.0.0 起 `/rd:release` 默认 draft，不再本地打 tag。如果用户先跑 `/rd:release v1.2.0`（draft 未 publish）→ 后跑 `/rd:changelog v1.3.0`，原来的 `git describe` 会返回更早的 tag（比如 `v1.1.0`），导致 v1.3.0 的 changelog 多包含了一整个版本的 commits。新回退链优先读平台 Release（含 draft）的 SHA，或在走 git tag 回退时警告用户。
...
- 重新生成：/rd:changelog <version> --from=<tag>
