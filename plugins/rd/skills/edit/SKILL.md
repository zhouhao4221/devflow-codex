...
执行模型：按[命令模型路由](../../shared/_command-models.md)中 `rd:edit` 的档位执行；已作为执行子代理时不再次路由。
...
快速修复（QUICK-XXX）：
  ...
  2. 插件模板：../../templates/quick-template.md
```
...
请执行 /rd:update-template <requirement|quick> 恢复模板
```
...
- 能独立交付 → 建议执行 /rd:new 创建新需求，在关联信息中引用当前 REQ
- 不能独立交付 → 继续在当前 REQ 中补充
```

**自动判断规则：**
- 原 REQ 已 `已完成` → 必须新建，提示用户执行 `/rd:new`
