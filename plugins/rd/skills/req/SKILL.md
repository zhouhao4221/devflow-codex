...
2. 实时扫描需求根目录 `active/`、`completed/` 和 `superseded/`，仅将 `REQ-XXX-*.md` 与 `QUICK-XXX-*.md` 作为需求文档。`INDEX.md` 不是事实源，不读取也不重写。
3. 解析每份文档的编号、标题、类型、模块、状态、更新时间、功能或验证进度及关联。状态与目录冲突时显式标注异常；不默默修正。
4. 按 `--module`、`--type` 过滤，分组展示活跃、已完成和已升级需求；已升级 QUICK 保留原状态，不计入已完成。头部显示插件版本、项目角色和分支策略；只显示实际可读到的数据。
