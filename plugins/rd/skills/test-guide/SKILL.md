---
name: test-guide
description: 测试引导助手。在执行 /rd:test、/rd:test-regression 或 /rd:test-new 命令时触发。 支持运行已有自动化测试和创建新测试用例。
---
...
| 命令 | 用途 | 说明 |
|-----|------|------|
| `/rd:test-regression` | 运行已有测试 | 执行现有自动化测试用例，回归验证 |
| `/rd:test-new` | 创建新测试 | 为新功能编写 UT/API/E2E 测试用例 |
| `/rd:test` | 综合测试 | 先运行回归、补充新测试，再对未覆盖项做能力感知交互走查 |
...
补充注入（存在则读，缺失静默跳过）：
- 运行 / 回归测试（`/rd:test`、`/rd:test-regression`）→ 复用已读取的 `docs/prompt/testing.md`
- 创建新用例（`/rd:test-new`）→ Read `docs/prompt/test-generation.md`（用例编写规范、覆盖要求）
...
- **readonly 仓库**：读取 `requirementSource.path` 指向主仓自身配置的 `<requirementsDir>/specs/`。未绑定时提示 `/rd:use`，不读全局缓存，不回写主仓。
...
⚠️ 项目中未检测到测试规范
   ...
   /rd:test 需要以下信息来定位和生成测试：
   - 测试文件位置和命名规则
   - 测试框架和运行命令
   - 测试环境搭建方式

   💡 添加方式：/rd:init <project> --reinit
```

---

## 一、回归测试 (`/rd:test-regression`)
...
```bash
/rd:test-regression              # 全量回归
/rd:test-regression --changed    # 仅测试变更相关
/rd:test-regression --module=xxx # 指定模块
/rd:test-regression --failed     # 仅运行上次失败的
```
...
💡 下一步：
- 修复问题后重新测试：/rd:test-regression --failed
```
