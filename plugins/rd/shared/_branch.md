...
分支策略存储在 `.devflow/settings.json` 的 `branchStrategy` 字段中，通过 `/rd:branch init` 初始化。`giteaToken` 敏感字段单独存入 `.devflow/settings.local.json`（不纳入 git）。
...
| 命令 | 读取的配置 | 用途 |
|------|-----------|------|
| `/rd:dev` | `branchFrom`、`featurePrefix`、`fixPrefix` | 创建分支时的基准和前缀 |
| `/rd:commit` | `mainBranch`、`developBranch` | 检查当前分支是否合规 |
| `/rd:done` | `mergeTarget`、`deleteBranchAfterMerge`、`repoType`、`giteaUrl` | 合并提醒、PR 创建（Gitea）|
| `/rd:branch hotfix` | `mainBranch`、`hotfixPrefix` | 从主分支创建紧急修复 |
| `/rd:branch status` | `repoType` | 显示仓库托管类型 |
