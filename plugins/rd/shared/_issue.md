...
```
- `GITEA_URL` 和 `TOKEN` 未配置时提示：`❌ Gitea 未配置 giteaUrl 或 giteaToken，请先执行 /rd:branch init`
...
```
feat/REQ-001-user-points-i12       ← /rd:dev，需求文档 issue=#12
fix/QUICK-003-fix-login-i5         ← /rd:dev，快速修复 issue=#5
fix/optimize-order-query-i42       ← /rd:do --from-issue=#42
fix/login-token-not-cleared-i42    ← /rd:fix --from-issue=#42
feat/REQ-001-user-points           ← 无 issue 关联，不加后缀
```

**规则**：
- `-iN` 仅当 issue 编号存在时追加（需求文档 `issue` 字段非 `-`，或 `/rd:do`、`/rd:fix` 的 `--from-issue` 参数）
- `N` 为纯数字，不带 `#`
...
| 优先级 | 来源 | 适用场景 |
|-------|------|---------|
| 1 | 需求文档元信息 `issue` 字段 | `/rd:done`、`/rd:commit`（有需求文档时） |
| 2 | 当前分支名的 `-iN` 后缀 | `/rd:commit`、`/rd:do`、`/rd:fix` 完成时（无需求文档时） |

**解析正则**：`-i(\d+)$` 匹配分支名末尾的 issue 编号。

### Issue 在 commit message 中的关联

当检测到 issue 编号时，`/rd:commit` 在 commit message 末尾追加 `closes #N`：
...
|------|-----------|---------|---------|
| `/rd:new --from-issue` | 需求文档 `issue` 字段 | `/rd:done` 询问 + API 关闭 | 需求完成时 |
| `/rd:new-quick --from-issue` | 需求文档 `issue` 字段 | `/rd:done` 询问 + API 关闭 | 需求完成时 |
| `/rd:do --from-issue` | 分支名 `-iN` | `/rd:do` 完成时询问 + API 关闭 | 任务完成时 |
| `/rd:fix --from-issue` | 分支名 `-iN` | `/rd:fix` 完成时询问 + API 关闭 | 修复完成时 |
