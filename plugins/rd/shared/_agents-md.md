...
在 dev/do/fix 生成方案前，优先读取项目 `docs/prompt/architecture.md` 获取分层、目录与契约；不存在时兼容读取 `AGENTS.md` 的架构章节。两者均缺失时从实际代码确定可验证事实，说明缺失，并可提示 `/rd:init --reinit`。不为通过检查重复询问已经明确的项目约定。
