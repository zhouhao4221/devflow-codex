...
| 选项 | 说明 | 示例 |
|-----|------|------|
| `--all` | 全量回归（默认） | `/req:test-regression --all` |
| `--changed` | 仅测试变更相关 | `/req:test-regression --changed` |
| `--module=<name>` | 指定模块 | `/req:test-regression --module=user` |
| `--failed` | 仅运行上次失败的 | `/req:test-regression --failed` |
| `--type=<ut\|api\|e2e>` | 指定测试类型 | `/req:test-regression --type=ut` |
| `--verbose` | 显示详细输出 | `/req:test-regression --verbose` |
| `--coverage` | 生成覆盖率报告 | `/req:test-regression --coverage` |
| `--skip-env` | 跳过环境检查（仅 UT） | `/req:test-regression --type=ut --skip-env` |
...
下一步操作：
- 修复后重新测试：/req:test-regression --failed
- 查看失败详情：/req:test-regression --verbose
...
```bash
/req:test-regression --ci --output=junit.xml
```
