...
| `plugins/uat/.codex-plugin/plugin.json` | `version` | uat 插件独立版本 |
...
| `plugins/diag/` | diag |
| `plugins/uat/` | uat |
| 其他（根目录、docs/、scripts/、.agents/ 等） | 不直接归属单个插件 |
...
### 4. 重新生成 Codex marketplace 文件

插件版本写入 `.codex-plugin/plugin.json` 后，运行生成脚本同步 commands 和 `.agents/plugins/marketplace.json`：
...
版本更新预览：
  ...
  uat:   1.2.0  → 1.3.0   (minor - 新增流程)
```

### 6. 更新版本文件

用 Edit 工具更新各 `.codex-plugin/plugin.json` 中的 `version` 字段。仅更新有变更的插件，然后运行生成脚本。

### 7. 暂存版本文件

```bash
git add .agents/plugins/marketplace.json plugins/*/.codex-plugin/plugin.json plugins/*/commands
```
