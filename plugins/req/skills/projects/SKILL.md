...
执行模型：按[命令模型路由](../../shared/_command-models.md)中 `req:projects` 的档位执行；已作为执行子代理时不再次路由。
...
读取当前仓库的 `.devflow/settings.local.json` / `.devflow/settings.json` 中的 `requirementProject`，legacy fallback 到 `.claude/settings.local.json`
