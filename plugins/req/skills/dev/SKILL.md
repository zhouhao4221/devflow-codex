...
> **前置**：读取项目 AGENTS.md 的「项目架构」章节，获取分层架构、目录结构、开发规范。
> 缺少时发出警告（见 _agents-md.md（见附录：_agents-md.md））。

进入 Plan Mode，基于需求文档和 AGENTS.md 架构信息生成实现方案，填充「十一、实现方案」章节：

- **11.1 数据模型**：新增/修改的表、字段说明、实体关系
- **11.2 API 设计**：基于第五章接口需求 + 项目代码 + AGENTS.md API 风格，生成具体接口方案（路径、方法、请求/响应字段、错误码）
- **11.3 文件改动清单**：按 AGENTS.md 分层架构表的顺序列出需要新增/修改的文件
- **11.4 实现步骤**：按 AGENTS.md 分层架构的顺序拆解开发步骤
...
按 AGENTS.md 分层架构表定义的顺序逐层开发。

实时检查（根据 AGENTS.md「开发规范」章节）：
- 文件命名规范
- AGENTS.md 中定义的其他规范项
