...
> **前置**：Read `docs/prompt/architecture.md`，获取分层架构、目录结构、开发规范；缺失则回退 AGENTS.md 的「项目架构」章节（兼容旧项目）。
> 两者都缺时发出警告（见 [_agents-md.md](../../shared/_agents-md.md)）。

先形成可审阅的方案，基于需求文档和已加载的项目架构信息生成实现方案，填充「十一、实现方案」章节：

> **定位现有代码**：方案需要参照的既有实现（相似模块、要改动的调用方、现有数据模型/接口）范围大且可独立定位时按共享规则派 `code-scout`（prompt 给：功能清单关键词、接口路径/实体名、architecture.md 的分层目录摘要），主会话只按其返回的 `file:line` 精读高/中相关片段。范围小、定位依赖主会话上下文或不可委派时直接搜索和读取。规则见 [`_delegate.md`](../../shared/_delegate.md)。
...
> 方案条目多（11.3 文件改动清单超过 10 行，或四个小节都要重写）时，回写交给 `doc-writer` subagent：prompt 内联已确认的方案要点 + 目标文件路径 + 四个小节的骨架，主会话核对实际写入内容和它返回的「要点核对」表。状态字段仍由主会话改。规则见 [`_delegate.md`](../../shared/_delegate.md)。
...
**同一层内按可独立验收的行为委派 `impl-worker`**（准入与拆分规则见 [`_delegate.md`](../../shared/_delegate.md) 的「实施准入」）：11.3 文件清单和 11.2 接口契约已确定时，把同一行为的实现与对应测试分给一个 owner，任务说明列明允许修改文件、只读依赖、相关规范和验收命令。无共享写入且值得委派的单元可并行；共享文件由主会话串行整合，主会话不同时编辑已派发文件。

**跨层不并行**：下一层依赖上一层定下的签名，必须等上一层复核通过再派。首次建立某个抽象/分层的那一版由主会话自己写，后续同构文件才委派。

每批 subagent 返回后主会话必须复核 `git diff` 实际内容（不是复核自述），按 [统一证据契约](../../shared/_evidence.md) 核对范围、当前内容与验证结果，逐条决策 `越界需求` / `存疑` 项，再进下一层。
...
每完成一步：更新任务状态（当前实施清单）。`primary` 仓库同时更新需求文档 checkbox，`readonly` 仓库仅更新任务状态，不修改需求文档。
