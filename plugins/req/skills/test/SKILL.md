...
4. **阶段一：UT** — 回归运行变更相关已有 UT → 缺失时引导 `/req:test-new --type=ut`
5. **阶段二：API 测试** — 按 testing.md 启动环境 → 回归已有 → 缺失时引导 `/req:test-new --type=api`
6. **阶段三：E2E 测试** — 额外检查前端服务 → 回归已有 → 缺失时引导 `/req:test-new --type=e2e`
