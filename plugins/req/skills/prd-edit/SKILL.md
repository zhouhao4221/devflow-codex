...
读取 `.devflow/settings.local.json` / `.devflow/settings.json` 的 `requirementRole`；若为 `readonly` 则报错退出，提示在主仓库执行。旧项目可 fallback 读取 `.claude/settings.local.json`。
