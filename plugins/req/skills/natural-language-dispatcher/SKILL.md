...
**执行动作**：
- 开启 → `mkdir -p .devflow && touch .devflow/.req-confirm-commit`，写入 memory
- 关闭 → `rm -f .devflow/.req-confirm-commit`，更新 memory
- 查询 → 检查 marker 是否存在，告知状态

> 只影响 命令执行确认，不拦截 Write/Edit。marker 已加入 `.gitignore`。
