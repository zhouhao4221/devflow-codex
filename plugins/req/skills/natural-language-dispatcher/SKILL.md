...
仅当用户显式传入 `--auto`，或明确要求修复后提交并推送创建 PR 时，映射为 `/req:fix --auto`。"一键修"、"自动修"、"不用确认"只影响已授权修复的交互，不增加提交、推送或 PR 授权；只要求其中部分操作时逐项传递授权，不用 `--auto` 补齐其他操作。用户和项目已有的分支、提交及 PR 偏好持续有效。
...
### 快速需求文档 → `/req:new-quick`

用户明确要求创建 QUICK / 快速需求文档时使用。单纯"快速修复"走 `/req:fix`，"加个小功能"走 `/req:do`；任务规模和"快速"措辞本身不代表要求创建文档。
