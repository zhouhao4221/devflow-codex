...
执行模型：按[命令模型路由](../../shared/_command-models.md)中 `req:status` 的档位执行；已作为执行子代理时不再次路由。
...
读取 `.devflow/settings.local.json` / `.devflow/settings.json` 的 `requirementProject` 和 `requirementRole`，legacy fallback 到 `.claude/settings.local.json`，按角色确定路径：
