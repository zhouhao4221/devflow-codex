...
初始化需求项目，创建本地需求存储目录和 DevFlow 通用配置，并绑定当前仓库。
...
- `--readonly`: 只读仓库角色，不创建本地需求正文目录，后续通过 `/req:use <primary-repo-path>` 绑定主仓
...
仓库角色优先级：`--readonly` 参数 → `.devflow/settings.local.json` 或 `.devflow/settings.json` 中已有 `readonly` → legacy `.claude/settings.local.json` → 默认 `primary`。

### 2. 创建目录结构

**primary**：`docs/requirements/` 下创建 `active/`、`completed/`、`modules/`、`templates/`

**readonly**：仅创建 `.devflow/` 配置目录和 `docs/requirements/templates/`，需求正文从 `requirementSource` 指向的主仓读取。
...
创建 `.devflow/` 目录。读取已有 `.devflow/settings.json`，合并字段后写回，不覆盖已有字段（如 `branchStrategy`）。
...
`--readonly` 未提供主仓路径时，提示用户执行 `/req:use <primary-repo-path>` 完成绑定。

### 7. Legacy Claude 兼容

仅当当前环境明确需要 Claude Code 兼容时，才合并写入 `.claude/settings.local.json` 的 `requirementProject` 和 `requirementRole`。不要把 `.claude/settings.local.json` 作为 DevFlow 默认配置源。

### 8. 生成架构文件

> 架构文件格式见 `plugins/req/templates/agent-snippets/`。新项目优先写入或补齐 `AGENTS.md`，

**8.1** `docs/prompt/architecture.md` 已存在则跳过
...
**8.3** 同时扫描目录分层、测试文件位置、代码风格，生成架构文件草稿，用户确认后写入

**8.4** 在 `AGENTS.md` 末尾追加架构文件引用（仅一行指针，不含内容）；不再创建或维护旧版工具专属入口。

### 9. 创建 Prompt 库骨架（仅当文件不存在）
...
创建 `.agents/skills/` 目录，根据项目类型引导创建通用项目 Skill；仅 Claude Code 兼容场景同步到 `.claude/skills/`：
...
成功时输出：
- 目录结构树（本地需求存储 + DevFlow 配置路径）
- 已生成的文件列表
- 下一步操作提示（检查架构文件 → 发版配置 → PRD → 分支策略 → 创建需求）

`--reinit` 模式额外标注「已存在」和「新增/补充」的区别。

`--readonly` 模式说明需求文档从 `requirementSource.path` 指向的主仓只读。
...
| readonly 未绑定主仓 | 提示执行 `/req:use <primary-repo-path>` |
| 权限不足 | 提示检查目录权限 |
