...
> 模板源文件：`plugins/req/templates/`，写入规范：`_storage.md`（见附录：_storage.md），索引格式：[`index-template.md`](../templates/index-template.md)，架构片段：[`agent-snippets/`](../templates/agent-snippets/)，release 模板：[`release-prompt-template.md`](../templates/release-prompt-template.md)，Prompt 库骨架：[`prompt-snippets/`](../templates/prompt-snippets/)
...
创建 `.agents/skills/` 目录，根据项目类型引导创建 Skill；仅 Claude Code 兼容场景同步到 `.claude/skills/`：
