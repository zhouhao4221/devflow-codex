...
执行模型：按[命令模型路由](../../shared/_command-models.md)中 `diag:diagnose` 的档位执行；已作为执行子代理时不再次路由。

通过自然语言描述报错 → 插件拉取生产日志 → AI 识别堆栈 → 在本地代码关联源码 → 输出诊断报告和修复建议。

**全程只读**，所有 SSH 命令都会经过 5 类风控校验，审计落盘。
...
2. 若上一轮 session 文件仍存在，读取旧 session id，对本次诊断涉及的每台主机各执行一次远端清理（`rm -f /tmp/devflow-diag-<OLD_SESSION>-*`）；此命令通过 write-guard 的 tmp 白名单，审计落盘
...
ssh <host> "tail -n <lines> <log_path> | grep -E '<pattern>' | tee -a /tmp/devflow-diag-<session>-<topic>.log"
# 之后读回
ssh <host> "cat /tmp/devflow-diag-<session>-<topic>.log"
# 清理（诊断结束时执行）
ssh <host> "rm -f /tmp/devflow-diag-<session>-*"
```

**命令约束**：
- 优先纯 stream（stdout 回本地），避免不必要的远端写入
- 远端临时文件路径必须以 `/tmp/devflow-diag-<session>-` 开头（`<session>` 即当前 session id）
- 写入只允许 `tee -a`（append），禁止裸 `>` / `>>`
- 清理只允许 `rm -f /tmp/devflow-diag-<session>-*`，禁止 `-r` / `-rf`
- 禁止在 `/tmp` 下创建子目录
...
通过 Bash 工具执行构造好的命令。风控链路会依次校验：
- `validate-guards`（首次校验完整性）
- `host-whitelist`（host 在白名单）
- `command-whitelist`（每个 verb 在白名单）
- `write-guard`（无写操作）

若任一风控校验拒绝 → 展示拒绝理由给用户，**不要**尝试绕过（如删选项、改写命令），而是向用户说明并询问是否调整需求。
...
审计：
  ...
  - 拉取日志：<log_paths>
  - SSH 命令：<3-5 条>
  - 审计记录：~/.devflow-diag/audit/command_audit-<date>.jsonl

未改动任何远程资源和本地代码。如需应用修复，你自己决定并手动执行。

Session 清理：
  - 远端临时文件：/tmp/devflow-diag-<session>-* 已清理 / 无遗留
  - 本地 session 文件：~/.devflow-diag/tmp/.current-session
```
...
- **不递归**：Codex 不应在本次会话中"为了调查而 SSH 到其他未登记主机"
...
| 现象 | 可能原因 |
|---|---|
| 风控拒绝 host | 服务未在 `services.yaml` 登记 → `/diag:init` 补充 |
| 风控拒绝命令 | AI 构造了非白名单命令，需要调整（如不要用 `tee`） |
