...
2. **创建目录**：`~/.devflow-diag/{config,audit}/`（700 权限）
3. **生成配置模板**：`~/.devflow-diag/config/services.yaml`（600 权限）
4. **引导配置**：让用户将真实服务登记到 `services.yaml`
5. **配置校验**：确保必填字段齐全
6. **（可选）空跑测试**：SSH `echo ok` 验证命令白名单链路
...
- 创建 `~/.devflow-diag/config/` 和 `~/.devflow-diag/audit/`
...
```
✅ 已创建配置模板：~/.devflow-diag/config/services.yaml
...
**用户配合方式**：
- **方式 A（对话录入）**：用户说"服务名 order-api，主机 prod-web-01，日志 /var/log/app/order.log"，AI 用 `Edit` 工具改 `~/.devflow-diag/config/services.yaml`
- **方式 B（自行编辑）**：用户说"我自己编辑"，AI 等待用户说"已填写"后继续校验

### 4. 校验配置

```bash
bash "${CODEX_PLUGIN_ROOT}/scripts/services-config.sh" validate
```
...
```
是否执行一次空测试？（SSH <host> 'echo ok' 验证命令白名单链路是否生效）  [Y/n]
```

若同意：
1. 从 `services.yaml` 取第一个服务的 host
2. 执行 `ssh <host> 'echo ok'`（应被命令白名单放行，因 echo 在白名单）
3. 展示退出码和 `~/.devflow-diag/audit/` 下的 JSONL 条目，证明链路通
...
配置：~/.devflow-diag/config/services.yaml（2 个服务）
审计：~/.devflow-diag/audit/
...
- `~/.devflow-diag/` 不纳入任何仓库；多机协作时，每台机器独立 init
...
| `services.yaml 已存在` | 脚本不覆盖，可手动 `rm ~/.devflow-diag/config/services.yaml` 后重 init |
| `命令白名单不生效` | 重新运行 `/diag:init` 并检查 `~/.devflow-diag/config/services.yaml` 配置 |
