...
启动前检测当前会话实际暴露的工具，**不要求 flow 文档预先指定，也不通过环境变量或客户端名称推断能力**。工具若要求先取得应用、浏览器或标签页清单，按其文档完成初始化调用后再选择表面；不同 Codex 运行时的工具名和参数可能不同，不臆造未暴露的接口。

| 能力或表面 | 优先用于 |
|-----------|---------|
| 用户明确指定的应用、浏览器或标签页 | 保留用户选择，不擅自切换会话 |
| Chrome 控制 | 需要现有登录态、标签页、浏览器扩展或用户配置的 web / mobile-web 测试 |
| 内置浏览器 | localhost、公开页面或适合独立会话的 web / mobile-web 测试 |
| Computer Use | 原生桌面应用、跨应用流程，或必须通过可视界面完成的步骤 |
| 截图 + 视觉分析 | 任意平台的视觉断言与失败证据 |
| Bash | CLI 工具、脚本、文件检查 |
| 无 UI 控制能力 | 提示用户手动操作，Codex 记录实际结果 |

工具选择只解决“怎么执行”，不改变模型路由：`uat:run` 的界面操作与视觉判断由 `primary` 主会话串行完成，不委派给经济档执行者。用户模型不支持已暴露工具时说明能力不匹配并降级，不静默切换模型。

若目标平台无可用 UI 控制能力：
```
⚠️  当前环境无可调用的 <目标平台> 控制能力，无法自动执行走查
💡  请在具备 Chrome 或 Computer Use 的 Codex 会话中重试，
    或按下方清单手动操作，由 Codex 记录实际结果
```
...
### 执行前：采集 console error（web 平台）

当前浏览器接口支持脚本求值时，在打开页面后、执行任何步骤前注入以下脚本，收集整个场景过程中的 console error：
...
每个场景结束后读取并清空 `window.__uatConsoleErrors`，作为该场景的附加信息。当前表面不支持脚本求值时记录“Console Error 未采集”，不要把缺少该能力判成业务失败，也不要声称控制台无错误。

### 对每个场景 `S0N`，按以下流程执行：

```
1. 输出场景标题：▶ S01 <场景名称>
2. 浏览器表面支持脚本求值时注入 console error 监听（见上方）
3. 数据准备：若有说明，提示用户手动准备后确认继续
4. 检查前置条件
5. 逐步理解并执行操作步骤（见三、意图驱动执行）
6. 逐条验证预期结果（见六、断言策略）
7. 已注入监听时读取并清空 window.__uatConsoleErrors；否则记录未采集
