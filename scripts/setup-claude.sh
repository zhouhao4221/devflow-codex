...
usage() {
  cat <<EOF
用法: $0 <devflow-codex路径> [devflow-claude路径]

将 devflow-codex 中的 79 个 SKILL.md 同步到 devflow-claude 的 plugins/*/skills/ 目录。

参数:
  devflow-codex路径      devflow-codex 仓库根目录路径（默认为当前目录）
...
echo "同步 devflow-codex → devflow-claude"
echo "  源头: $SKILLS_SRC"
echo "  目标: $SKILLS_DST"
echo ""
