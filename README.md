...
命令按工作类型选择执行模型：`release`、查询和规则化操作优先经济档（Luna）；报告和常规生成使用标准档（Terra）；需求、开发和设计保留主会话模型。直接调用 skill 同样生效。主会话仍负责交互、验收和已授权的外部操作；运行环境不支持模型覆盖时会说明并回退。详见[命令模型路由](shared/command-model-routing.md)。
