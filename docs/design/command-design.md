...
| 规则化状态/配置：req-review、done、config | 经济档；校验当前状态、目标状态和允许写入的资源。模型价格与写入授权分开判断 |
...
2026-09-15 的后续适配将大 PR 的抽象审查后端落到 Codex 官方非交互 review，同时保留固定 SHA、只读、无隐式评论和失败降级约束；设计与验证记录见 [Codex 原生代码审查适配](../changes/2026-09-15-codex-native-review.md)。

同日将 `req:test` 的手动交互验证改为能力感知走查：运行时检查实际 UI 控制表面，按登录态与目标平台选择 Chrome、内置浏览器或 Computer Use；UI 操作和最终验收保持 `primary`，机械测试运行仍可降档。设计与边界见 [能力感知的交互走查](../changes/2026-09-15-capability-aware-ui-testing.md)。

2026-09-16 将命令档位解释为有界执行工作的默认模型，主会话先识别当前模型并负责方案与验收；Astra 或 5.6 Sol 作主模型时按任务难度使用 5.6 worker。具体决策与回退见 [主会话判断与 5.6 执行分工](../changes/2026-09-16-primary-worker-routing.md)。
