...
后续按 Codex 原生多代理运行方式进一步调整：按需继承上下文、共享目录内按行为单元分配实现与测试、区分运行中消息和空闲代理续接、消费完成通知并避免重复派发。任务状态和测试结论分开，复用验证时核对相关工作区内容。详见 [子任务委派](../../plugins/rd/shared/_delegate.md) 和 [证据契约](../../plugins/rd/shared/_evidence.md)。
